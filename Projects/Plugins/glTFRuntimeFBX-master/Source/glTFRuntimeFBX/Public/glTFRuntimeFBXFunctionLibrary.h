...
USTRUCT(BlueprintType)
struct FglTFRuntimeFBX_PreLoadInfo /// xrdev add 
{
	GENERATED_BODY()
public:
	FglTFRuntimeFBX_PreLoadInfo()
	{
		bBlenderCreateor = false;
		bSkeletonMesh = false;
	}

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "glTFRuntime_ForXR|FBX")
	bool bBlenderCreateor; // 블렌더에서 만들어 졌는지 ..

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "glTFRuntime_ForXR|FBX")
	bool bSkeletonMesh; // 스케레톤 매시 인지.
};
...
public:
	...
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "glTFRuntime|FBX")
	static bool GetFbxInfo(FString fbxFilePath, FglTFRuntimeFBX_PreLoadInfo & fbxInfo); // xrdev mod plugin
};
