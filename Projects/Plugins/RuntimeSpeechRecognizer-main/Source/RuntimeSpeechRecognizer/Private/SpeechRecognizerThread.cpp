...
bool IsMostlySilence(const Audio::FAlignedFloatBuffer& Buffer, float Threshold = 0.04f)
{
	if (Buffer.Num() == 0) return true;
	...
	float RMS = FMath::Sqrt(SumSquares / Buffer.Num());
...
uint32 FSpeechRecognizerThread::Run()
{
	while (!GetIsStopped() && !GetIsStopping())
	{
		...
		{
			// If the sound is close to silence, the process will not run.
			if (IsMostlySilence(NewQueuedBuffer))
			{
				UE_LOG(LogRuntimeSpeechRecognizer, Log, TEXT("Speech Recognizer Testing, Skip Buffer : Skipped NewQueuedBuffer"));
				continue;
			}
			else
			{
				UE_LOG(LogRuntimeSpeechRecognizer, Log, TEXT("Speech Recognizer Testing, Skip Buffer : NOT skipped NewQueuedBuffer"));
			}
