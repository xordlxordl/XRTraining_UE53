  ...
  <div id="sidebar-wrapper" class="flex-column flex-shrink-0 p-3 text-bg-dark">
    ...
    <ul class="sidebar-nav nav nav-masthead flex-column mb-auto">
      ...
      <hr>
		...
		<li class="nav-item"><a href="dr_user_main.jsp" class="nav-link text-white">훈련자</a></li>
			<li class="nav-item">
				<a class="nav-link text-white fs-4 active" href="#">수정</a>
			</li>
      <hr>
    </ul>
  </div>
		...
		<div class="container">
			<div class="row mb-3">
                ...
                <div class="form-row text-bg-light mx-auto">
					...
					<form class="text-center" method="post" action="dr_UpdateUserAction.jsp">
						...
						<div class="row input-group-text">
							...
							<select id="city" class="form-select mx-3" name = "nsi" onchange="changeCityFn()" required>
								...
								for( i = 0; i < list.size(); ++i )
								{
							  	...
							  	%> 
							  	 value=<%= i+1 %> > <%= list.get(i).getName() %></option>
							  	<%
								}
							  	%>
							...
							<select id="gungu" class="form-select mx-3" name = "ngun" onchange="changeGunGuFn()" required>
								...
								ArrayList<DRJuso> list2 = db2.getJuso2( user.getNsi() );
								int i2;
								for( i2 = 0; i2 < list2.size(); ++i2 )
								{
							  	...
							  	<option
							  	<%
							  		if( user.getNgun() == list2.get(i2).getNid() )
							  		{
							  			%>
							  			selected
							  			<%
							  	...
							  	%> 
							  	 value=<%= list2.get(i2).getNid() %> > <%= list2.get(i2).getName() %></option>
							  	<%
								}
							  	%>
							</select>
							...
							<select id="dong" class="form-select" name = "ndong" required>
							  	<option value="">읍/면/동</option>
							  		<%
							  	DRDBWork db3 = new DRDBWork();
								ArrayList<DRJuso> list3 = db3.getJuso3( user.getNgun() );
								int i3;
								for( i3 = 0; i3 < list3.size(); ++i3 )
								{
							  	...
							  	<option
							  	<%
							  		if( user.getNdong() == list3.get(i3).getNid() )
							  		{
							  			%>
							  			selected
							  			<%
							  	...
							  	%> 
							  	 value=<%= list3.get(i3).getNid() %> > <%= list3.get(i3).getName() %></option>
							  	<%
								}
							  	%>
							</select>
						...
						<div class="row input-group-text">
							...
							{
							%>
								<input type="text" class="form-control text-center" name="szsick" id="sicktext" placeholder=""
								value="<%= user.getSzsick() %>" 
								>
							<%
							}
							%> 
						...
						<div class="row mt-2 input-group-text">
							...
							<div class="form-group col-md-6">
								...
								{
								%>
								<input type="text" class="form-control text-center me-5" name ="szdrug" id="drugtext" placeholder="" 
								value="<%=user.getSzdrug() %>">
								<%
								}
								%>
							...
							<div class="form-group col-md-4 mx-0">
			              	...
			              	<label class="mx-1">사고경험: &nbsp; &nbsp; </label>
								...
								<div class="form-check form-check-inline dr_checkbox">
									...
									{
									%>
									<div class="row mt-2 input-group-text"  id = "crash_option">

									<div class="col-md-5 offset-md-2">
			              			<div class="input-group-lg " >
									<label class="mx-1 text-dark " id="crash_text">사고유형:&nbsp; &nbsp;</label >
									<div class="form-check form-check-inline">
									<input class="form-check-input dr_checkbox" type="checkbox" name="bcrashobject" id="inlineCheckCrashObject" value="1"
									 <% if( user.getBcrashobject() == 1 )
										{
										%>
										checked
										<%
									...
									<div class="form-check form-check-inline">
									<input class="form-check-input dr_checkbox" type="checkbox" name="bcrashhuman" id="inlineCheckCrashHuman" value="1"
									<% if( user.getBcrashhuman() == 1 )
										{
										%>
										checked
										<%
									...
									<div class="form-check form-check-inline dr_checkbox">
									<input class="form-check-input dr_checkbox" type="checkbox" name="bcrashonly" id="inlineCheckCrashOnly" value="1"
										<% if( user.getBcrashonly() == 1 )
										{
										%>
										checked
										<%
						...
						<div class="row mt-2">
							...
							<div class="col-md-3">
	                        	<button class="btn btn-danger" onclick ="CancelUpdateUser()" type="button">취소</button>
							</div>
...
if( request.getParameter("myresult") != null )
{
	...
	{		
		...
		{
%>
		<script>MessageBox( '<%=request.getParameter("name")%>' + ' 수검자 수정 성공');</script>
<%		
		}
