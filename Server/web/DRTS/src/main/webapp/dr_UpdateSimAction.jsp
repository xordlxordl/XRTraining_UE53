...
<jsp:useBean id="droption" class="data.DROption" scope="page"/>
<jsp:setProperty name="droption" property="uid"/>
<jsp:setProperty name="droption" property="mode"/>
<jsp:setProperty name="droption" property="timezone"/>
<jsp:setProperty name="droption" property="weather"/>
<jsp:setProperty name="droption" property="amount"/>
<jsp:setProperty name="droption" property="windspeed"/>
<jsp:setProperty name="droption" property="winddir"/>
<jsp:setProperty name="droption" property="bAEBS"/>
...
<head>
	...
	<link rel="stylesheet" href="css/SetCircleSlide.css">
...
<%
	...
	Integer nResult = db.SetOption( droption );
	...
	{
		%>
		<script>MyError('DB 프로시저 오류 NULL');</script>
		<%
	}
	DRUser user = new DRUser();
	DRDBWork db2 = new DRDBWork();
	String uid = ((Integer)droption.getUid()).toString();  
	user = db2.GetTraineeData( uid ) ;
			              ...
			              <div class="row input-group-text dr_simulation">
								...
								<div class="form-group col-md-4 text-start">
				              	<div class="input-group-sm input-group-text">
									...
									<div class="form-check form-check-inline">
										<input class="form-check-input" type="radio" name="mode" id="id_practice_mode" value="1"
										<%
										if( droption.getMode() == 1)
										{
										%>
										checked
										<%
									...
									<div class="form-check form-check-inline">
										<input class="form-check-input" type="radio" name="mode" id="id_driving_mode" value="2"
										<%
										if( droption.getMode() == 2)
										{
										%>
										checked
										<%
...
<!--2 row------------------------------------------------------------------------------------------------------------------------------->
						<div class="row dr_simulation">
							<div class="form-group col-md-8">
								<div class="row input-group-text mt-1 me-0">
									...
									<label class="float-start">시&nbsp;&nbsp;간&nbsp;&nbsp;대:</label>
										...
										<div class="form-group col-7 mx-5">
						              		<div class="input-group-sm input-group-text">

			
												<div class="form-check form-check-inline">
													<input class="form-check-input" type="radio" name="timezone" id="idtimeam" value="1"
													<%
													if( droption.getTimezone() == 1 )
													{
													%>
													checked
													<%
								 ...
								 <div class="row input-group-text mt-1 me-0">
									...
									<div class="form-group col-7 mx-5 text-start ">
						              	<div class="input-group-sm input-group-text">
											...
											<div class="form-check form-check-inline">
												<input class="form-check-input" type="radio" name="weather" id="idweathersun" value="1"
													<%
													if( droption.getWeather() == 1 )
													{
													%>
													checked
													<%
								...
								<div class="col-md-9 card mx-2">
									<div class="row mt-1 mx-0">
										...
										<div class="col-md-7">
										<input type="range" min="0" max="100" class="form-range mt-2 dr_amount" id="IDRangeAmount" oninput="IDEditAmouont.value = IDRangeAmount.value" 
										value="<%=droption.getAmount() %>" disabled>
										</div>
										<div class="col-md-3">
										<input type="number" min="0" max="100" class="form-control form-control-sm text-center mt-1 dr_amout" id="IDEditAmouont" name="amount" placeholder="0~100" 
										value="<%=droption.getAmount() %>" disabled>
										</div>
										...
										value="<%=droption.getWindspeed() %>" disabled>
										</div>
										<div class="col-md-3">
										<input type="number" min="0" max="45" class="form-control form-control-sm text-center mt-1" id="IDEditWind" name="windspeed" placeholder="0~45" 
										value="<%=droption.getWindspeed() %>" disabled>
										</div>
									</div>
							...
							<div class="col-md-4 mt-1">
								<div class="row input-group-text">
									...
									<div class="col-4">
										<label>풍향</label>
										<input type="text" class="form-control form-control-sm text-center" id="IDEditWindPos" name="winddir" placeholder="" readonly
										value="<%=droption.getWinddir() %>" disabled>
									</div>			
						...
						<div class="row mt-1">
							...
							%>
								<div class="col-md-3">
								<button class="btn btn-primary" onclick ="RestartPracticeMode();" type="button">Restart</button>
								</div>
							<%
							}
							%>
							...
							<div class="col-md-3">
	                        	<button class="btn btn-warning text-white" onclick ="ResetDrivingMode(<%=droption.getUid() %>)" type="button">Reset</button>
							</div>
...
<script src="js/SetCircleSlide.js"></script>
