...
<%
	...
	{
		%>		
		<script>UpdateUserProdMessageOK2('<%= ituserprod.getPuid() %>번 유저 제품 수정 완료', '<%= ituserprod.getUid() %>')</script>
		<%
	}
	else if( nResult == 0 )
	{
		%>
		<script>UpdateUserProdError2('실패','<%= ituserprod.getPuid() %>');</script>
		<%
	}
	else if( nResult == -2 )
	{
		%>
		<script>UpdateUserProdError2('제품 ID 중복','<%= ituserprod.getPuid() %>');</script>
		<%
	}
