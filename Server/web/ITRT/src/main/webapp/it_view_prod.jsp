...
<div id="page-wrapper">
  ...
  <div id="sidebar-wrapper" class="flex-column flex-shrink-0 p-3 text-bg-dark">
    ...
    <ul class="sidebar-nav nav nav-masthead flex-column mb-auto">
      ...
      <li class="sidebar-brand">
		...
		<hr>
	      ...
	      <li class="nav-item"
      		...
      		data-bs-custom-class="custom-tooltip"
      		data-bs-title="유저 ID: [ <%=uid%> ] 에게 제품을 등록합니다."      
	      
	      ><a href="it_add_userprod2.jsp?uid=<%=uid%>" class="nav-link text-white" >유저 제품 등록</a></li>
      <hr>
      <span class="fs-3"
      data-bs-toggle="tooltip"
      data-bs-placement="top"
      ...
      >수정메뉴</span>
      	<li class="nav-item" id="id_modyfymenu" style="display:none"><a onclick="GoToUserProdEdit()" class="btn btn-primary" type="button">수정</a></li>
  ...
  <!-- /사이드바 -->
	...
	<section>
		<div class="container">
			<div class="row mb-5">
				<div class="col-md-12 col-xl-9 text-center mx-auto">
                    <h1>유저 제품 이용리스트</h1>
                </div>
            </div>
			<table id="id_maintable" class="table table-striped table-bordered table-dark table-hover" style="table-layout:fixed ; border:1px solid #fff">
					<thead>
					<tr>
						<th class="bg-white text-black text-center" >Index</th>
						<th class="bg-white text-black text-center" >유저 ID</th>
						<th class="bg-white text-black text-center" >제품 ID</th>
						<th class="bg-white text-black text-center" >제품 이름</th>
						<th class="bg-white text-black text-center" >시작 시간</th>
						<th class="bg-white text-black text-center" >끝 시간</th>
				...
				<tbody id="id_maintbody" style="display:none">
						...
						ArrayList< ITUserProductData > list = db.GetUserProdDataList( uid );
						int i;
					...
					<tr>
						...
						<td class="text-center"><%= list.get(i).getPid() %></td>
						<td class="text-center"><%= list.get(i).getProdname() %></td>
						
						<td class="text-center"><%= list.get(i).getCreateat() %></td>
