...
<%
	...
	Integer nResult = db.AddOrg( itorg );
	...
	if( nResult == 1 )
	{
		...
		String x;
		x = "location.href = 'it_add_org.jsp?myresult=1&name=" + itorg.getName() + "'";
		script.println( x );
	...
	{
		%>
		<script>AddOrgError('실패');</script>
		<%
	}
	else if( nResult == -1 )
	{
		%>
		<script>AddOrgError('조직 이름 중복');</script>
		<%
	}
	else if( nResult == -3 )
	{
		%>
		<script>AddOrgError('없는 조직');</script>
		<%
	}
%>
