  ...
  <div id="sidebar-wrapper" class="flex-column flex-shrink-0 p-3 text-bg-dark">
    ...
    <ul class="sidebar-nav nav nav-masthead flex-column mb-auto">
      ...
      <hr>
		...
		<li class="nav-item"><a href="it_org_main.jsp" class="nav-link text-gray">조직</a></li>
		<li class="nav-item"><a href="it_user_main.jsp" class="nav-link text-white ">유저</a></li>
		<li class="nav-item"><a class="nav-link text-white fs-4 active" href="it_add_user.jsp">등록</a></li>
      <hr>
    </ul>
  </div>
  <!-- /사이드바 -->
            ...
            <div class="row d-flex justify-content-center">
                <div class="col-md-6 col-xl-4">
                    <div class="card my-5 text-bg-light">
                    	<div class="card-header"> <h3>유저 수정</h3></div>
                        <div class="card-body d-flex flex-column align-items-center">
                            <div class="my-3"></div>
                            
                            <form class="text-center" method="post" action="it_UpdateUserAction.jsp">
                                
                                <div class="mb-3"><input class="form-control text-white bg-black" type="number" name="uid" placeholder="UID" maxlength="15" readonly value="<%=user.getUid()%>"></div>
                                
								<div class="mb-3">
								<select id="city" class="form-select" name = "oid" required>
							  	<option value="">조직 이름</option>
							  	<%
							  	ITDBWork db = new ITDBWork();
								ArrayList<ITOrg> list = db.GetOrgList();
								int i;
								for( i = 0; i < list.size(); ++i )
								{
							  	%>
							  	<option value=<%=list.get(i).getOid()%> 
							  	<%
							  	if( user.getOid() == list.get(i).getOid() )
							  	{
							  	%>
							  	selected
							  	<%
                                ...
                                <div class="mb-3"><input class="form-control" type="text" name="loginid" placeholder="로그인 계정" maxlength="15" required
                                value="<%=user.getLoginid() %>"></div>
                                <div class="mb-3"><input class="form-control" type="text" name="pass" placeholder="비밀번호" maxlength="15"></div>
                                <div class="bd-example">
                                <button class="btn border-white text-white bg-primary btn-outline-light"  type="submit">수정</button>
                                <button class="btn btn btn-outline-danger" onclick ="CancelUpdateUser()" type="button">취소</button>
                                </div>
                            </form>
...
if( request.getParameter("myresult") != null )
{
	...
	{		
		...
		{
%>
		<script>MessageBox( '<%=request.getParameter("name")%>' + ' 계정 생성 성공');</script>
<%		
		}
