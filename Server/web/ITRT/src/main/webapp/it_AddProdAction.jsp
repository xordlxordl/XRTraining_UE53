...
<%
	...
	Integer nResult = db.AddProd( itprod );
	...
	if( nResult == 1 )
	{
		...
		String x;
		x = "location.href = 'it_add_prod.jsp?myresult=1&name=" + itprod.getName() + "'";
		script.println( x );
	...
	{
		%>
		<script>AddProdError('실패');</script>
		<%
	}
	else if( nResult == -1 )
	{
		%>
		<script>AddProdError('제품 이름 중복');</script>
		<%
	}
	else if( nResult == -3 )
	{
		%>
		<script>AddProdError('없는 조직');</script>
		<%
	}
%>
