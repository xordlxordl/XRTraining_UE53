...
<%
	...
	Integer nResult = db.DeleteOrg( uid );
	
	if( nResult == null )
	{
		%>
		<script>OrgMessageError('DB 오류 Data NULL');</script>
		<%
		return;
	}
	...
	{
		%>
		<script>UpdateOrgMessageOK( '<%=username%>조직 삭제 완료' );</script>
		<%
	}
	else if( nResult == 0 )
	{
		%>
		<script>OrgMessageError('실패');</script>
		<%
	}
