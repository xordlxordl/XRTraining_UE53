...
<%
	...
	Integer nResult = db.DeleteUser( uid );
	
	if( nResult == null )
	{
		%>
		<script>UserMessageError('DB 오류 Data NULL');</script>
		<%
		return;
	}
	...
	{
		%>
		<script>UpdateUserMessageOK( '<%=username%>계정 삭제 완료' );</script>
		<%
	}
	else if( nResult == 0 )
	{
		%>
		<script>UserMessageError('실패');</script>
		<%
	}
