...
<body class="h-75 text-center text-bg-dark">
	...
	{
		Result = db.UpdateTrainerWithOutPass( user.getUid(), user.getUserid(), user.getUsernumber(), user.getUsername());
	}
	else
	{
		Result = db.UpdateTrainer( user.getUid(), user.getUserid(), user.getUsernumber(), user.getUsername(), user.getUserpassword() );	
	}
	...
	if( result == 1 )
	{
		...
		*/
		%>
		<script>Main2MessageOK( '<%=user.getUsername()%> 수정 성공' );</script>
		<%
	}
	else if( result == 0 )
	{
