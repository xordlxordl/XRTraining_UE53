...
<body class="h-75 text-center text-bg-dark">
	...
	String strAdminID = null;
		...
		User ResultUser = db.joinTrainer( strAdminID, user.getUserid(), user.getUsernumber(), user.getUsername(), user.getUserpassword() );
		if( ResultUser == null )
		{
	%>
		...
		int result = ResultUser.getResult();
		...
		if( result == 1 )
		{
			...
			script.println("<script>");
			String x;
			x = "location.href = 'join_trainer.jsp?myresult=1&name=" + user.getUsername() + "'";
			script.println( x );
			script.println("</script>");
		}
		else if( result == 0 )
