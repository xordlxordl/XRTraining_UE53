...
<jsp:setProperty name="xrs" property="mapid"/>
<jsp:setProperty name="xrs" property="name"/>
<jsp:setProperty name="xrs" property="ip"/>
<jsp:setProperty name="xrs" property="port"/>
...
<body class="h-75 text-center text-bg-dark">
	...
	int result = db.UpdateServer( xrs.getNid(), xrs.getMapid(), xrs.getName(), xrs.getIp(), xrs.getPort());
			
	if( result == 1 )
	{
		
		%>
			<script>ServerMessageOK( '<%=xrs.getNid()%> 번 서버 수정 성공' );</script>
		<%
	}
	else if( result == 0 )
	{
