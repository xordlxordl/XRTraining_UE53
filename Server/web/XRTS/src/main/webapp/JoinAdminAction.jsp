...
<body class="h-75 text-center text-bg-dark">
	...
	String strAdminID = null;
		...
		User ResultUser = db.joinAdmin( strAdminID, user.getUserid(), user.getUsernumber(), user.getUsername(), user.getUserpassword() );
		if( ResultUser == null )
		{
	%>
		...
		if( result == 1 )
		{
			...
			script.println("<script>");
			String x;
			x = "location.href = 'join_admin.jsp?myresult=1&name=" + user.getUsername() + "'";
			script.println(x);
			script.println("</script>");
