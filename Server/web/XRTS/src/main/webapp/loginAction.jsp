...
<jsp:useBean id="cadmin" class="data.CAdmin" scope="page"/>
<jsp:setProperty name="cadmin" property="adminid"/>
<jsp:setProperty name="cadmin" property="adminpass"/>
...
<body class="h-75 text-center text-bg-dark">
	...
	int result = 0;
		...
		if( strAdminID != null && iAdminGrade != 100 )
		{
		...
		CAdmin adminResult = db.login( cadmin.getAdminid(), cadmin.getAdminpass() );
		...
		else
		{
			...
			adminResult.setAdminid( cadmin.getAdminid() );
			
			result = adminResult.getResult();
			if( result == 1 )
			{
		
		session.setAttribute("adminid", adminResult.getAdminid());
		session.setAttribute("admingrade", adminResult.getAdmingrade());
		
		PrintWriter script = response.getWriter();
		script.println("<script>");
		String x;
		
		if( adminResult.getAdmingrade() < 2 )
			x = "location.href = 'main.jsp?myresult=1&name=" + adminResult.getAdminid() + "'";
		else
			x = "location.href = 'main2.jsp?myresult=1&name=" + adminResult.getAdminid() + "'";
