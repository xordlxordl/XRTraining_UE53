...
<head>
	...
	<script src="js/myTable2.js"></script>
    ...
    <ul class="sidebar-nav nav nav-masthead flex-column mb-auto">
     ...
     <%
      ...
      %>
		...
		<li class="nav-item"><a href="server_main.jsp" class="nav-link text-white fs-4 active">서버</a></li>
		<li class="nav-item"><a href="add_server.jsp" class="nav-link text-white">등록</a></li>
		<hr>
	 ...
	 %>
      ...
      <span class="fs-3">수정메뉴</span>
      	<li class="nav-item" id="id_modyfymenu" style="display:none"><a href="#" onclick="GoToServerEdit()" class="nav-link text-white">수정</a></li>
	...
	<section>
		<div class="container">
			...
			<table id="id_maintable" class="table table-striped table-bordered table-dark table-hover" style="table-layout:fixed ; border:1px solid #fff">
				<thead>
					<tr>
						<th class="bg-white text-black">ID</th>
						<th class="bg-white text-black">시나리오 ID</th>
						<th class="bg-white text-black">이름</th>
						<th class="bg-white text-black">IP</th>
						<th class="bg-white text-black">PORT</th>
						<th class="bg-white text-black">실행 유무</th>
					</tr>
				</thead>
				<tbody id="id_maintbody" style="display:none">
						...
						ArrayList<Server> list = db.GetServerList();
						int i;
					...
					<tr>
						...
						<td><%= list.get(i).getPort() %></td>
						<td><%= list.get(i).getBon() %></td>
					</tr>
						<%
							}
						%>
