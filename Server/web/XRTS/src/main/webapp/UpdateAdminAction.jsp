...
<body class="h-75 text-center text-bg-dark">
	...
	{
		Result = db.UpdateAdminWithOutPass( user.getUid(), user.getUserid(), user.getUsernumber(), user.getUsername());
	}
	else
	{
		Result = db.UpdateAdmin( user.getUid(), user.getUserid(), user.getUsernumber(), user.getUsername(), user.getUserpassword() );	
	}
	...
	if( result == 1 )
	{
		...
		%>
			<script>Main1MessageOK( '<%=user.getUsername()%> 수정 성공' );</script>
		<%
	}
	else if( result == 0 )
	{
