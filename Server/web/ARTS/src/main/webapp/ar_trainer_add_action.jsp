...
<jsp:setProperty name="aradmin" property="num"/>
...
<%
	...
	Integer result = db.spAddTrainer( aradmin );
	
	if( result == 1 )
	{
		%>
		<script>AddTrainerOK( '<%=aradmin.getAccount()%> 추가 성공');</script>
		<%
	}
	else if( result == 0 )
	{
	...
	{
		%>
		<script>MyError2Line('이미 있는 관제자입니다','입력한 데이터를 확인 후 다시 진행 해주세요');</script>
		<%
	}
	else if( result == -2 )
	{
	...
	{
		%>
		<script>MyError2Line('이미 있는 계정입니다','입력한 데이터를 확인 후 다시 진행 해주세요');</script>
		<%
	}
	else if( result == -6 )
	{
