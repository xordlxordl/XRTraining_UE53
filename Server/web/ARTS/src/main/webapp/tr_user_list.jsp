...
<head>
	...
	<script src="js/myTableTRUser.js"></script>
...
<%
	...
	arDBWork dbtitle = new arDBWork();
	arTrTitle Title = dbtitle.spGetTrTitle(tid);
...
<header class="navbar navbar-expand-md navbar-dark bg-dark" aria-label="Dark offcanvas navbar">
	<div class="container-fluid">
		...
		<div class="navbar-brand"><div class="fs-1 col-md-2">교육 관리><div class="fs-2 col-md-1 d-inline">훈련 보기</div></div></div>
		...
		<div class="container">
			<div class="row mb-5">
				<div class="col-md-8 col-xl-9 text-center mx-auto">
                    <h1>
                    <%= Title.getTitle()%>&nbsp;<%=Title.getStartTime()%>&nbsp;(시작 시간)
			...
			<table id="id_maintable" class="table table-striped table-bordered table-dark table-hover compact" style="table-layout:fixed ; border:1px solid #fff">
				<thead>
					<tr>
						...
						<th class="bg-white col-md-2 text-black">보직</th>
						<th class="bg-white col-md-2 text-black">기기</th>
						<th class="bg-white text-black">훈련자</th>
						<th class="bg-white col-md-2 text-black">군번</th>
						<th class="bg-white col-md-2 text-black">상세보기</th>
					</tr>
				</thead>
				<tbody id="id_maintbody" style="display:none">
						...
						ArrayList< arTrUserData > list = db.spGetTRUserList( tid );
						int i;
					...
					<tr>
						...
						<td> <button type="button" class="btn btn-outline-primary col-md-6 btn-ssm text-white" style="cursor: pointer;" 
						onclick="GoToTRUserDetail(<%=tid%>,<%=list.get(i).getUID()%>);">상세보기</button></td>
					</tr>
						<%
						}
						%>
