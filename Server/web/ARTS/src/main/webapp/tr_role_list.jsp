...
<head>
	...
	<script src="js/myTableRole.js"></script>
...
<header class="navbar navbar-expand-md navbar-dark bg-dark" aria-label="Dark offcanvas navbar">
	<div class="container-fluid">
		<a class="navbar-brand" href="index.jsp"> <p class="fs-1">보직 목록</p> </a>
...
<div id="page-wrapper">
  ...
  <div id="sidebar-wrapper" class="flex-column flex-shrink-0 p-3 text-bg-dark">
       ...
       <ul class="sidebar-nav nav nav-masthead flex-column mb-auto">
		...
		<li class="nav-item"><a href="tr_role_list.jsp" class="nav-link text-white fs-4 active">보직 목록</a></li>
		<li class="nav-item"><a href="tr_role_add.jsp" class="nav-link text-white">보직 추가</a></li>
      	<%
        }
      	%>
    </ul>
	...
	<section>
		<div class="container">
			...
			<table id="id_maintable" class="table table-striped table-bordered table-dark table-hover compact" style="table-layout:fixed ; border:1px solid #fff">
				<thead>
					<tr>
						<th class="bg-white text-black">보직ID</th>
						<th class="bg-white text-black">보직명</th>
						<th class="bg-white text-black">수정</th>
					</tr>
				</thead>
				<tbody id="id_maintbody" style="display:none">
						...
						ArrayList<arRole> list = db.spGetRoleList();
						int i;
					...
					<tr>
						...
						<td><%= list.get(i).getRName() %></td>
						
						<td> <button type="button" class="btn btn-outline-primary col-md-6 btn-ssm text-white" style="cursor: pointer;" onclick="GoToUserEditWithIndex(<%=list.get(i).getRID()%>);">수정하기</button></td>
					</tr>
						<%
						}
						%>
