...
<head>
	...
	<script src="js/myMCHTable.js"></script>
...
<header class="navbar navbar-expand-md navbar-dark bg-dark" aria-label="Dark offcanvas navbar">
	<div class="container-fluid">
		<a class="navbar-brand" href="index.jsp"> <p class="fs-1">장비 관리</p> </a>
...
<div id="page-wrapper">
  ...
  <div id="sidebar-wrapper" class="flex-column flex-shrink-0 p-3 text-bg-dark">
       ...
       <ul class="sidebar-nav nav nav-masthead flex-column mb-auto">
		...
		<!-- <li class="nav-item"><a href="ar_mch_list.jsp" class="nav-link text-white active">장비 목록</a></li> -->
		<%
		if( iAdminGrade < 2 )
        {
      	%>
	...
	<section>
		<div class="container">
			...
			<table id="id_maintable" class="table table-striped table-bordered table-dark table-hover compact" style="table-layout:fixed ; border:1px solid #fff">
				<thead>
					<tr>
						<th class="bg-white text-black col-sm-1">nID</th>
						<th class="bg-white text-black">장비그룹명</th>
						<th class="bg-white text-black">백팩PC</th>
						<th class="bg-white text-black">HMD</th>
						<th class="bg-white text-black">총기트래커</th>
						<th class="bg-white text-black">허리트래커</th>
						<th class="bg-white text-black">왼발트래커</th>
						<th class="bg-white text-black">오른발트래커</th>
						<th class="bg-white text-black">수정하기</th>
					</tr>
				</thead>
				<tbody id="id_maintbody" style="display:none">
						...
						ArrayList< arMachine > list = db.spGetMachineList();
						int i;
					...
					<tr>
						...
						<td><%= list.get(i).getBPC() %> </td>
						<td><%= list.get(i).getHMD() %> </td>
						
						<td><%= list.get(i).getUTT1() %> </td>
						<td><%= list.get(i).getUTT2() %> </td>
						<td><%= list.get(i).getUTT3() %> </td>
						<td><%= list.get(i).getUTT4() %> </td>

						<td> <button type="button" class="btn btn-outline-primary col-md-6 btn-ssm text-white" style="cursor: pointer;" onclick="GoToMCHEditWithIndex(<%=list.get(i).getnID()%>);">수정하기</button></td>
					</tr>
						<%
						}
						%>
