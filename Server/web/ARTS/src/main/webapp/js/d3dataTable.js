...
$(document).on("change", "#csv", function()
{
	...
	const fileText = fileInput.files[0];
	
	const reader = new FileReader();
	const readerText = new FileReader();
	
	readerText.readAsText( fileText, "utf-8");
	readerText.onload = () =>
	{
		const result = readerText.result;

		if( result.includes('�'))
		{
            ErrorMsg('UTF-8로 인코딩 되지 않은 CSV파일입니다');
            return;
        }
        else
        {
